...
class SqlAlchemyCaptureRepository(CaptureRepository):
    ...
    def save(self, capture: Capture) -> None:
        model = CaptureModel(id=capture.id, content=capture.content, created_at=capture.created_at)
    ...
    def get(self, capture_id: UUID) -> Capture | None:
        ...
        if not model:
            return None
        return Capture(id=model.id, content=model.content, created_at=model.created_at)

    def list(self) -> list[Capture]:
        models = self.session.exec(select(CaptureModel)).all()
        return [Capture(id=model.id, content=model.content, created_at=model.created_at) for model in models]
